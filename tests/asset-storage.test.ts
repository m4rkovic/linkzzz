...
import { InvalidImageError, validateImage } from "@/server/assets/asset-storage";
...
test("asset validation rejects spoofed and unsupported content with a typed client error", () => {
  assert.throws(
    () => validateImage(new TextEncoder().encode("not a png"), "image/png"),
    InvalidImageError,
  );
  assert.throws(
    () => validateImage(new TextEncoder().encode("<svg></svg>"), "image/svg+xml"),
    InvalidImageError,
  );
});
