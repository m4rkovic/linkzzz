...
function profileData(row: ProfileGraph): PersistedProfileData {
  const appearance = row.appearance as PersistedProfileData["appearance"] & { __media?: { avatarUrl?: string; coverImageUrl?: string } };
  return {
    slug: row.slug, displayName: row.displayName, username: row.username ?? undefined, bio: row.bio,
    locationLabel: row.locationLabel ?? undefined, status: row.status, appearance,
    avatarUrl: appearance.__media?.avatarUrl, coverImageUrl: appearance.__media?.coverImageUrl,
...
export class PrismaProfileRepository implements ProfileRepository {
  constructor(private readonly db: PrismaClient) {}
  async findByUserId(userId: string) { const row = await this.db.profile.findUnique({ where: { userId }, include: profileInclude }); return row ? profileData(row) : null; }
  ...
  async upsert(userId: string, data: PersistedProfileData) {
    ...
    await this.db.$transaction(async (tx) => {
      ...
      const profile = await tx.profile.upsert({ where: { userId }, create: { userId, ...base }, update: base });
