...
export default function ChangePasswordModal({
  ...
  return (
    <DialogShell
      open={open}
      onClose={forced ? undefined : resetAndClose}
      closeOnEscape={!forced && !loading}
      closeOnBackdrop={!forced && !loading}
      initialFocusRef={firstInputRef}
      ariaLabelledby="change-password-title"
      ariaDescribedby="change-password-description"
      overlayClassName="z-[80]"
      panelClassName="max-h-[92vh] overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:max-w-lg sm:rounded-3xl"
    >
      <div className="flex items-start justify-between gap-4 border-b border-zinc-100 p-5 sm:p-6">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p id="change-password-description" className="mt-1 text-sm leading-5 text-zinc-500">Update the password used to access this account.</p>
          </div>
        </div>

        {!success && !forced && (
          <button type="button" onClick={resetAndClose} disabled={loading} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-zinc-500 transition hover:bg-zinc-100 disabled:opacity-50" aria-label="Close change password dialog">
            <X size={18} />
          </button>
        )}
      </div>
              ...
              <div className="mt-3 space-y-2">
                {rules.map((rule) => (
                  <div key={rule.id} className="flex items-center gap-2 text-sm">
                    <span className={`flex h-5 w-5 items-center justify-center rounded-full ${rule.passed ? "bg-emerald-100 text-emerald-700" : "bg-zinc-200 text-zinc-400"}`} aria-hidden="true">
