...
import {
  ...
  EditorSection,
  EditorSubtabs,
} from "./link-editor-primitives";

type Props = { draft: LinkDraft; onChange: (values: Partial<LinkDraft>) => void };

type BasicsTab = "content" | "layout";

const basicsTabs = [
  { id: "content", label: "Content", description: "Text and destination" },
  { id: "layout", label: "Card layout", description: "Size and proportions" },
] as const;

export default function LinkBasicSections({ draft, onChange }: Props) {
  const [activeTab, setActiveTab] = useState<BasicsTab>("content");
  ...
  return (
    <div>
      <EditorSubtabs
        label="Basic card settings"
        tabs={basicsTabs}
        activeTab={activeTab}
        onChange={(tab) => setActiveTab(tab as BasicsTab)}
      />
